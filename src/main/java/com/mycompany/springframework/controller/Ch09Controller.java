...
public class Ch09Controller {
  ...
  /*
   * @PostMapping("/fileUpload") public String fileUpload(String title, String desc, MultipartFile
   * attach) throws IOException { log.info("제목: " + title); log.info("설명: " + desc); // 파일의 정보 얻기
   * log.info("원래 파일 이름: " + attach.getOriginalFilename()); log.info("파일의 종류: " +
   * attach.getContentType()); log.info("파일의 크기: " + attach.getSize()); // 영구적 파일로 저장 File destDir =
   * new File("C:/Temp/uploadFiles"); if (!destDir.exists()) { destDir.mkdirs(); } File destFile =
   * new File(destDir, new Date().getTime() + "-" + attach.getOriginalFilename());
   * 
   * attach.transferTo(destFile); return "redirect:/ch09/fileUploadForm"; }
   */
  ...
  public String fileUpload(Ch09FileUploadForm form) throws IOException {
    ...
    // 파일의 정보 얻기
    log.info("원래 파일 이름: " + form.getAttach().getOriginalFilename());
    ...
    File destFile =
        new File(destDir, new Date().getTime() + "-" + form.getAttach().getOriginalFilename());
